...
#define LOG_PASSED printf("[" KGRN "PASSED" KNRM "] %s (%d tests)\n", __FUNCTION__, passed_count);
#define LOG_PINFO(_fmt, _args...) printf("[" KCYN "INFO  " KNRM "] %s " _fmt "\n", __FUNCTION__, _args);

typedef int (test_myslave_test_handler) ();

#define BEGIN_TEST(_name) static int _name() { int passed_count = 0;
#define BEGIN_TESTA(_function) static int _function { int passed_count = 0;
